...
public interface PersonMapper
{
	...
	@Select(
	{ "SELECT id, firstName, lastName, parent_id, parent_firstName, parent_lastName", "FROM Person", "WHERE id = #{id,jdbcType=INTEGER}" })
	...
	{ "SELECT id, firstName, lastName, parent_id, parent_firstName, parent_lastName", "FROM Person", "WHERE id = #{id,jdbcType=INTEGER}" })
	@Results(
	{ @Result(id = true, column = "id", property = "id"), @Result(property = "parent", column = "{firstName=parent_firstName,lastName=parent_lastName}", one = @One(select = "getParentWithParamAttributes"))
	
	})
	Person getComplexWithParamAttributes(Long id);
	
	@Select("SELECT id, firstName, lastName, parent_id, parent_firstName, parent_lastName" + " FROM Person" + " WHERE firstName = #{firstName,jdbcType=VARCHAR}" + " AND lastName = #{lastName,jdbcType=VARCHAR}" + " LIMIT 1")
